...
export default function Blog() {
    ...
    console.log(blogId)
    const { data: BlogData, isLoading, error, isError } = useGetBlogQuery(blogId, { refetchOnMountOrArgChange: true });
            ...
            <div className="w-full max-w-4xl mx-auto py-12 px-4 md:px-6">
                <article className="prose prose-gray max-w-none dark:prose-invert">
                    <div className="space-y-4 not-prose">
                        ...
                        <div className="flex items-center gap-4" onClick={() => navigate(`/profile/${BlogData?.data.author}`)}>
                            <a href="#" className="flex items-center gap-2 text-sm font-medium" >
                                <Avatar className="w-8 h-8 border border-black" >
                                    <AvatarImage src="https://avatar.iran.liara.run/public" />
                                    <AvatarFallback>AC</AvatarFallback>
                                </Avatar>
                                Avatar -later figure out names
                            </a>
                            <div className="text-gray-500 dark:text-gray-400 text-sm">Posted on {formatDate(BlogData?.data?.createdAt as Date)}</div>
                        </div>
                    </div>
